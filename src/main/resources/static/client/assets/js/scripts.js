...
// home page 2 Main menu ofcanvas
                            ...
                            if (data) {// User is authenticated
                                ...
                                for (let x of data) {
                                    $('.offcanvas-cart-list').find('.added-cart-list').append('<div class="single-added-list">\n' +
                                        ...
                                        '                    <img src="/api/product-image/get/' + x.productDetailResponse.productResponse.productImageResponseSet[0].id + '" alt="" style="height: 80px; width: 80px; object-fit: contain">\n' +
                                        '                    <div class="single-item-details midea-body">\n' +
                                        '                        <h5><a href="#">' + x.productDetailResponse.productResponse.name + '</a></h5>\n' +
                                        '                        <p>' + getNumberWithCommas(x.productDetailResponse.productResponse.salePrice) + ' X ' + x.quantity + '</p>\n' +
                                        '                    </div>\n' +
                                        '                </div>\n' +
                                        ...
                                        '                    <h6>' + getNumberWithCommas(x.productDetailResponse.productResponse.salePrice*x.quantity) + ' VND</h6>\n' +
                                        '                </div>\n' +
                                        '                <!-- End of Single cart list right -->\n' +
                                        '            </div>')
                                    totalCost += x.productDetailResponse.productResponse.salePrice*x.quantity;
                                }
    ...
    $('.add-to-bag, .addto-icon-btn a').on('click', function (e) {
        // var productImageSrc = $(this).closest('.product-thumb').find('.hover-state').attr('src');
        // $('.modal-add-notifacition').find('img').attr('src',productImageSrc);
        //
        // var productName = $(this).closest('div.product-buttons').next().find('a').text();
        // $('.modal-add-notifacition').find('div span').text(productName);

        //check if not sign in yet
        var userId = $('.modal-quickview-container').attr('user-id');
        if (userId == 0){
            $('.offcanvas-account').addClass('show');
            $('.offcanvas-overlay').css({
                'opacity': 1,
        ...
        //get product image
        var productImageSrc1 = $(this).closest('.product-thumb').find('img.normal-state').attr('src');
        var productImageSrc2 = $(this).closest('.product-thumb').find('img.hover-state').attr('src');
        //get product name
        var productName = $(this).closest('.product-buttons').next().find('a').html();
        //get product Id
        var productId = $(this).attr("data-id");
        //get product size
        $.ajax({
            method: "get",
            url: "/client/size/" + productId,
            success: function (data) {
                //insert size data
        ...
        $.ajax({
            ...
            success: function (data) {
                //insert color data
                $('.modal-quickview-container').find('.color-group').find('ul').empty();
                for (let x of data) {
                    $('.modal-quickview-container').find('.color-group').find('ul').append('<li><a href="#" class="color" color-id="' + x.id + '" style="background-color: ' + x.hexCode + '"><span></span></a></li>');
                }
                //event handle: choose a color
                $('.color-group a').on('click', function (e) {
                    e.preventDefault();
                    $(this).parent().parent().find('a').removeClass('active');
                    $(this).addClass('active');
                })
        ...
        //get product price and product image
        $.ajax({
            method: "get",
            url: "/client/product/" + productId,
            success: function (data) {
                //insert product price
                $('.new-price').text(getNumberWithCommas(data.salePrice) + ' VND');
                $('.old-price').text(getNumberWithCommas(data.costPrice) + ' VND');

                //insert product image
                $('.modal-quickview-container').find('div.quick-product-gallery').children().empty();
                for (let x of data.productImageResponseSet){
                    $('.modal-quickview-container').find('div.quick-product-gallery').children().append(
                        '<div class="swiper-slide">\n' +
                        '    <img class=\'\' src="/api/product-image/get/' + x.id + '" alt=""\n' +
                        '       data-rjs="2" style="width: 540px; height: 600px; object-fit: contain"/>\n' +
                        '</div>'
                    );
                }

                //insert slider image
                $('.modal-quickview-container').find('div.quick-product-thumbs').children().empty();
                for (let x of data.productImageResponseSet){
                    $('.modal-quickview-container').find('div.quick-product-thumbs').children().append(
                        '<div class="swiper-slide">\n' +
                        '    <img src="/api/product-image/get/' + x.id + '" data-rjs="2" alt="">\n' +
                        '</div>'
                    );
                }
        ...
        //product name
        $('.modal-quickview-container').find('.product-title a').html(productName);
        //product id
        $('.modal-quickview-container').find('.product-title a').attr('data-id',productId);
    ...
    //event handle: add to cart
    $('.addto-bag-btn').click(function (e) {
        e.preventDefault();

        var userId = $('.modal-quickview-container').attr('user-id');

        var quantity = $(this).closest('.product-info').find('input.product-quantity-list').val();
        var productId = $(this).closest('.product-info').find('.product-title a').attr('data-id');
        var selectedSize = $(this).closest('.product-info').find('.product-size.active').attr('size-id');
        var selectedColor = $(this).closest('.product-info').find('a.color.active').attr('color-id');

        // addProdcuttoCard(quantity, productId, selectedSize, selectedColor);

        var url = "/api/shoppingcarts/add1/" + productId + "/" + quantity + "/" + userId + "/" + selectedSize + "/" + selectedColor ;
        $.ajax({
            type: "POST",
            url: url,
        }).done(function (res) {
            //get product image
            var productImageSrc = $('.modal-quickview-container').find('div.quick-product-gallery').children().find('div:first').children().attr('src');
            //get productname
            var productName = $('.modal-quickview-container').find('.product-title a').html();
    ...
    // function addProdcuttoCard(quantity, productId, size, color) {
    //     var userId = [[${userId}]];
    //
