...
public class UserServiceImpl implements UserService {
	...
	public UserResponse add(UserRequest userRequest) {
		...
		if (userRepository.existsByEmail(userRequest.getEmail())) throw new AppException(ErrorCode.EMAIL_DUPLICATE);
		// Map to entity
	...
	public UserResponse deactivate(Long id) {
		...
		// Save & Return
		return userMapper.toUserResponse(userRepository.save(oldUserEntity));
	}
	...
	@PreAuthorize(
			"hasRole('ADMIN') or hasAuthority('UPDATE_INFORMATION') or (#userRequest.id == authentication.principal.claims['uid'])")
	@Override
	...
	@PreAuthorize(
			"hasRole('ADMIN') or hasAuthority('CHANGE_PASSWORD') or (#request.id == authentication.principal.claims['uid'])")
	@Override
