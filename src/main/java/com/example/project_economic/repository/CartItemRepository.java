...
//    CartItemEntity findByUserAndProductAttribute(UserEntity user, ProductEntity product, String size, String color);
    ...
    @Query(value = "SELECT * FROM cart_items c WHERE c.user_id = :userId AND c.product_with_attributes_id = :productWithAttributesId ", nativeQuery = true)
    CartItemEntity findByUserAndProductAttribute(@Param("userId") Long userId, @Param("productWithAttributesId") Long productWithAttributesId);

    @Query(value = "SELECT * FROM cart_items c WHERE c.user_id = :userId", nativeQuery = true)
    Set<CartItemEntity> findByUserId(@Param("userId") Long userId);
