...
public class CartItemController {
	...
	public ResponseEntity<ResponseObject> showCartSizeOfAnUser(@PathVariable Long userId) {
		...
		return buildResponse(HttpStatus.OK, "Cart-size fetch successfully.", cartItemService.countAllByUserId(userId));
	}

	@PostMapping("/add")
	public ResponseEntity<ResponseObject> add(
			@Validated({Create.class, Default.class}) @RequestBody CartItemRequest cartItemRequest) {
		return buildResponse(HttpStatus.OK, "Added cart-item successfully.", cartItemService.add(cartItemRequest));
	}

	@PutMapping("/update")
	public ResponseEntity<ResponseObject> update(
			@Validated({Update.class, Default.class}) @RequestBody CartItemRequest cartItemRequest) {
		return buildResponse(HttpStatus.OK, "Updated cart-item successfully.", cartItemService.update(cartItemRequest));
	}
